...
The Windows packaging is done by the win-build.xml ANT file. This uses the Oracle javafxpackager. For the generation
of the Windows exe it must be run on a Windows machine.
As inputs it uses the two directories:
target/windows/staging
target/windows/multibit-exe.dir

These are generated by doing a "mvn clean package" as described above.

You can either run the full build on your Windows machine or copy these work files onto your Windows machine
and then run the ANT file to create the Windows installer.

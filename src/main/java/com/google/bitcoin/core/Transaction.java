 ...
 * <p>
 * 
 * It implements TWO serialization protocols - the BitCoin proprietary format
 * which is identical to the C++ implementation and is used for reading/writing
 * transactions to the wire and for hashing. It also implements Java
 * serialization which is used for the wallet. This allows us to easily add
 * extra fields used for our own accounting or UI purposes.
 * <p>
 * 
...
public class Transaction extends ChildMessage implements Serializable, IsMultiBitClass {
    ...
    // This is being migrated to appearsInHashes. It's set to null after
    // migration.
    Set<StoredBlock> appearsIn;

    // Stored only in Java serialization. This is either the time the
    // transaction was broadcast as measured from the
    // local clock, or the time from the block in which it was included. Note
    // that this can be changed by re-orgs so
    // the wallet may update this field. Old serialized transactions don't have
    // this field, thus null is valid.
    // It is used for returning an ordered list of transactions from a wallet,
    // which is helpful for presenting to
    // users.
    Date updatedAt;
    ...
    // has been included in. For most transactions this set will have a single
    // member. In the case of a chain split a
    // transaction may appear in multiple blocks but only one of them is part of
    // the best chain. It's not valid to
    // have an identical transaction appear in two blocks in the same chain but
    // this invariant is expensive to check,
    ...
    public Transaction(NetworkParameters params, int version, Sha256Hash hash) {
        super(params);
        this.version = version & ((1L << 32) - 1); // this field is unsigned -
     ...
     * wallet. The flag controls whether to include spent outputs or not.
     */
    ...
    /**
     * Returns a set of blocks which contain the transaction, or null if this
     * transaction doesn't have that data because it's not stored in the wallet
     * or because it has never appeared in a block.
     */
    ...
    /**
     * Puts the given block in the internal serializable set of blocks in which
     * this transaction appears. This is used by the wallet to ensure
     * transactions that appear on side chains are recorded properly even though
     * the block stores do not save the transaction data at all.
     * <p>
     * 
     * <p>
     * If there is a re-org this will be called once for each block that was
     * previously seen, to update which block is the best chain. The best chain
     * block is guaranteed to be called last. So this must be idempotent.
     * 
     * <p>
     * Sets updatedAt to be the earliest valid block time where this tx was seen
     * 
     * @param block
     *            The {@link StoredBlock} in which the transaction has appeared.
     * @param bestChain
     *            whether to set the updatedAt timestamp from the block header
    ...
    public void setBlockAppearance(StoredBlock block, boolean bestChain) {
        ...
        if (bestChain) {
            // This can cause event listeners on TransactionConfidence to run.
            // After this line completes, the wallets
    ...
    /**
     * Called by the wallet once a re-org means we don't appear in the best
     * chain anymore.
     */
    ...
    /**
     * Connects all inputs using the provided transactions. If any input cannot
     * be connected returns that input or null on success.
     */
    TransactionInput connectForReorganize(Map<Sha256Hash, Transaction> transactions) {
        maybeParse();
        for (TransactionInput input : inputs) {
            // Coinbase transactions, by definition, do not have connectable
            // inputs.
            if (input.isCoinBase())
                continue;
            ...
            if (result == TransactionInput.ConnectionResult.SUCCESS)
                continue;
            // The input doesn't exist in the wallet, eg because it belongs to
            // somebody else (inbound spend).
    ...
    protected void parseLite() throws ProtocolException {
        ...
        // skip this if the length has been provided i.e. the tx is not part of
        // a block
        if (parseLazy && length == UNKNOWN_LENGTH) {
            // If length hasn't been provided this tx is probably contained
            // within a block.
            // In parseRetain mode the block needs to know how long the
            // transaction is
            // unfortunately this requires a fairly deep (though not total)
            // parse.
            // This is due to the fact that transactions in the block's list do
            // not include a
            // size header and inputs/outputs are also variable length due the
            // contained
            // script so each must be instantiated so the scriptlength varint
            // can be read
            // to calculate total length of the transaction.
            // We will still persist will this semi-light parsing because
            // getting the lengths
            // of the various components gains us the ability to cache the
            // backing bytearrays
            // so that only those subcomponents that have changed will need to
            // be reserialized.
     ...
     * output. Note that this input is NOT complete and after every input is
     * added with addInput() and every output is added with addOutput(),
     * signInputs() must be called to finalize the transaction and finish the
     * inputs off. Otherwise it won't be accepted by the network.
     */
    ...
    public void addOutput(TransactionOutput to) {
        ...
        // these could be merged into one but would need parentTransaction to be
        // cast whenever it was accessed.
        to.setParent(this);
    ...
    /**
     * Once a transaction has some inputs and outputs added, the signatures in
     * the inputs can be calculated. The signature is over the transaction
     * itself, to prove the redeemer actually created that transaction, so we
     * have to do this step last.
     * <p>
     * <p/>
     * This method is similar to SignatureHash in script.cpp
     * 
     * @param hashType
     *            This should always be set to SigHash.ALL currently. Other
     *            types are unused.
     * @param wallet
    ...
    public void signInputs(SigHash hashType, Wallet wallet) throws ScriptException {
        ...
        for (int i = 0; i < inputs.size(); i++) {
            ...
            // This assert should never fire. If it does, it means the wallet is
            // inconsistent.
            assert key != null : "Transaction exists in wallet that we cannot redeem: "
                    + Utils.bytesToHexString(connectedPubKeyHash);
            ...
            // Now sign for the output so we can redeem it. We use the keypair
            // to sign the hash,
            // and then put the resulting signature in the script along with the
            // public key (below).
            try {
                // usually 71-73 bytes
    ...
    /**
     * Ensure object is fully parsed before invoking java serialization. The
     * backing byte array is transient so if the object has parseLazy = true and
     * hasn't invoked checkParse yet then data will be lost during
     * serialization.
     */
